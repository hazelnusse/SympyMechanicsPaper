...
purposefully choose a nonminimal set of coordinates and speeds.  In this
particular example, this can be avoided, but in more complicated systems, it is
difficult (if not impossible) to describe the system with a minimal choice.  The
orientation of the disc fixed frame $C$ relative to the inertial frame $N$ is given by
a sequence of angular rotations (Euler ZXY) by angles $q_0$ (yaw), $q_1$
(lean), $q_2$ (spin).  The location
of the center of disc relative to the inertial origin is defined by coordinates
q3, q4, q5.  The angular velocity of the disc is defined by three speeds
\[u_0 \triangleq {^{N}}\bm{\omega}^{C} \cdot \bm{c}_{x}\]
