...
\begin{table}[htbp]
  ...
  \begin{tabular}[c]{l l l}
    ...
    \hline
    $\bm{q}$ & $\mathbf{R}^n$ & Coordinates \\
    $\bm{\dot{q}}$ & $\mathbf{R}^n$ & Time derivatives of coordinates \\
    $\bm{u}$ & $\mathbf{R}^o$ & Speeds \\
    $\bm{\dot{u}}$ & $\mathbf{R}^o$ & Time derivatives of speeds \\
  ...
  \begin{tabular}[c]{r @{ $=$ } l l}
    ...
    \hline
    $\bm{f}_{c}(\bm{q}, t)$ & $\bm{0}_{l \times 1}$ & Configuration (holonomic)
    constraints \\
    $\bm{f}_{v}(\bm{q}, \bm{u}, t)$ & $\bm{0}_{m \times 1}$ & Velocity
    ...
    \bm{u}, \bm{r}, t)$ & $\bm{0}_{(o - m) \times 1}$ & Dynamic differential
    equations \\
  \end{tabular}
...
\begin{align}
    \prime{A} &\triangleq \tilde{M}^{-1} \tilde{A} \\
    \prime{B} &\triangleq \tilde{M}^{-1} \tilde{B}
\end{align}

Now we will have matrices $\prime{A}$ and $\prime{B}$ which are of size $(o +
n) \times (o - m + n - l)$ and $(o + n) \times p)$. We need to now extract only
the rows which are associated with the independent quantities. We will define
% Is there a better way to write transforming A' to A, B' to B?
% Could we use the permutation matrices here?
the reduced matrices as $A$ and $B$. We will also define the state space vector
$x = \left[\delta\bm{q}_i,\,\delta\bm{u}_i\right]^{T}$; we now have the
...
Additionally, there are two requirements for this method to work, namely, both
$\nabla_{\bm{q}}\bm{f}_{c} P_{qd}$ and $\nabla_{\bm{u}} \bm{f}_{v} P_{ud}$
